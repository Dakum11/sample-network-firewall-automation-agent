...
@lru_cache(maxsize=1)
def _get_ipam_config():
    """Lazily load IPAM configuration from Secrets Manager. Cached after first call."""
    secret_name = os.getenv('IPAM_SECRET_NAME', 'ipam/credentials')
    credentials = json.loads(get_secret(secret_name))
    username = credentials["username"]
    password = credentials["password"]
    solidserver_url = credentials["ipam_url"]
    ...
    return solidserver_url, headers


def _get_solidserver_url():
    """Get the SOLIDSERVER_URL (lazy loaded)."""
    url, _ = _get_ipam_config()
    return url


def _get_headers():
    """Get the HEADERS (lazy loaded)."""
    _, headers = _get_ipam_config()
    return headers
...
def describe_subnet_owner_region(
    subnet_object: dict, cidr_range: str = None, ipaddress: str = None
) -> str:
    """
    ...
    params = dict(
        item.split("=", 1) for item in params_decoded.split("&") if "=" in item
    )
...
def check_cidr_range(cidr_range: str) -> dict:
    ...
    """
    solidserver_url = _get_solidserver_url()
    ...
    if response.ok:
        if response.text:
            ...
            return {
                "status": "success",
                "exists": True,
                "message": describe_subnet_owner_region(data[0], cidr_range=cidr_range),
            }
        else:
            return {
                "status": "success",
                "exists": False,
                "message": f"The CIDR range {cidr_range} entry does not exist in IPAM",
            }
    ...
    if response.ok:
        if response.text:
            ...
            return {
                "status": "success",
                "exists": True,
                "message": describe_subnet_owner_region(data[0], ipaddress=ip_address),
            }
        else:
            return {
                "status": "success",
                "exists": False,
                "message": f"{ip_address} entry does not exist in IPAM",
            }
    return {"status": "error", "error": response.text}
